...
// Visual flowchart editor component
...
const FlowchartEditor: React.FC<FlowchartEditorProps> = ({ data, updateData }) => {
    ...
    const initializePositions = () => {
        ...
        data.document.procedures[0].sections.forEach(section => {
            const sectionSteps: EnhancedStep[] = [];
            section.steps.forEach(step => {
                sectionSteps.push(step as EnhancedStep);
            });
            sections.push(sectionSteps);
        });
    ...
    const handleRemoveConnection = (fromStepId: number, toStepId: number) => {
        ...
        // If from step is a decision point, remove from decision paths
        if (fromStep.is_decision_point && fromStep.decision_paths) {
            // Check all decision paths
            newData.document.procedures[0].sections.forEach(section => {
                section.steps.forEach(step => {
                    if (step.step_number === fromStepId) {
                        const updatedStep = step as EnhancedStep;

                        if (updatedStep.decision_paths) {
                            // Update each path
                            updatedStep.decision_paths.forEach(path => {
            ...
            newData.document.procedures[0].sections.forEach(section => {
                section.steps.forEach(step => {
                    if (step.step_number === fromStepId) {
                        ...
                        if (updatedStep.next_steps) {
                            const index = updatedStep.next_steps.indexOf(toStepId);
                            if (index !== -1) {
                                updatedStep.next_steps.splice(index, 1);
                                updated = true;
                            }
                        }
                    }
